  ...
  window.addEventListener("load", () => {
    ...
    totalBands.forEach((band, index) => {
      gsap.to(band, {
        ...
        scrollTrigger: {
          ...
          start: `right right-=${
            (totalBands.length - index) * band.offsetWidth
          }px`,
    ...
    function showcaseNavbarAnimation() {
      ...
      function animateElement(target) {
        const translateXValue = `${
          showcaseNav.offsetParent.offsetWidth - showcaseHeader.offsetWidth
        }px`;
        const startValue = `left left+=${
          showcaseNav.offsetParent.id.replace("sec-", "") * bandWidth
        }`;
        const endValue = `right right-=${
          (totalBands.length -
            showcaseNav.offsetParent.id.replace("sec-", "")) *
          bandWidth
        }`;
      ...
      const showcaseNavLinks = gsap.utils.toArray(
        "#showcase-nav a.showcase-nav-link"
      );
      const showcasePanels = gsap.utils.toArray(
        "#showcase-panels > div.slide-group"
      );
    ...
    document.querySelectorAll(".section .nav-link").forEach((link) => {
      link.addEventListener("click", (e) => {
        e.preventDefault();
        document
          .querySelector(`.band[data-section="${link.getAttribute("href")}"]`)
          .click();
      });
    });
    ...
    document.querySelectorAll(".scroll-indicator a").forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        document
          .querySelector(
            `.band[data-section="${e.target.getAttribute("href")}"`
          )
          .click();
      });
    });
